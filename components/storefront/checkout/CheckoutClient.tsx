...
export default function CheckoutClient({
  ...
  const detailLabel = [offer.region_code, denominationLabel]
    .filter(Boolean)
    .join(" • ");
  ...
  const emailError =
    email.length > 0 && !validEmail(email)
      ? t("checkout.errors.invalidEmail") || "Email inválido"
      : null;
  const whatsappError =
    whatsapp.length > 0 && !/^\d{8,}$/.test(whatsapp)
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (json.error) {
        ...
        const successPath =
          typeof json.successPath === "string" ? json.successPath : null;
        if (!successPath) {
                  ...
                  <option value="">
                    {province
                      ...
                      : t("checkout.selectProvinceFirst") ||
                        "Seleccione a província primeiro"}
                  </option>
            ...
            <div className="space-y-3">
              {paymentMethods.length === 0 ? (
                ...
                paymentMethods.map((pm) => (
                  <div
                    key={pm.id}
                    className={`p-4 rounded-lg border transition-all cursor-pointer ${selectedPayment === pm.id ? "border-zuma-500 bg-zuma-50 dark:bg-zuma-900/20" : "border-borderc bg-card hover:border-zuma-200"}`}
                    onClick={() => onPaymentChange(pm.id)}
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-4 h-4 rounded-full border flex items-center justify-center ${selectedPayment === pm.id ? "border-zuma-500" : "border-muted"}`}
                      >
                        {selectedPayment === pm.id && (
                          <div className="w-2 h-2 rounded-full bg-zuma-500" />
                        )}
                      </div>
                    ...
                    {selectedPayment === pm.id && pm.type === "manual" && (
                      <div className="mt-4 pt-4 border-t border-borderc/50 pl-7">
                        <div className="text-sm text-muted">
                          {t("checkout.instructionsLabel")}:
                        </div>
                        {pm.instructions_md ? (
                          <div className="mt-2 text-sm text-text whitespace-pre-wrap bg-bg p-3 rounded border border-borderc">
                            {pm.instructions_md}
                          </div>
                        ) : (
                          <div className="mt-2 text-sm text-muted">
                            {t("checkout.noAdditionalInstructions")}
                          </div>
                        )}
                        ...
                        <div className="mt-4">
                          <label className="flex items-start gap-2 text-sm cursor-pointer">
                            <input
                              type="checkbox"
                              aria-label={t("checkout.confirmCopied")}
                              className="mt-1 rounded text-zuma-500 focus:ring-zuma-500"
