...
export default function OrderSuccessClient({ accessToken, whatsappNumber, order, items }: { accessToken: string, whatsappNumber: string, order: any, items: any[] }) {
    ...
    try {
      ...
      const lines = [
        ...
        ...items.map((item: any) => {
          ...
          const denomination = `${item?.offer?.denomination_currency ?? ''} ${item?.offer?.denomination_value ?? ''}`.trim()
      ...
      const res = await fetch(`/api/orders/${encodeURIComponent(accessToken)}/handoff`, { method: 'POST' })
