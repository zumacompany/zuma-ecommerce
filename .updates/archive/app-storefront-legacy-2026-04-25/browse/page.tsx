                        ...
                        {filteredBrands.map((b) => (
                            ...
                            >
                                <div className="h-12 w-full flex items-center justify-center relative">
                                    {b.logo_path ? (
                                        <Image src={b.logo_path} alt={`${b.name} logo`} fill sizes="(max-width: 768px) 33vw, 200px" className="object-contain" />
                                    ) : (
...
function BrowsePageFallback() {
    return (
        <main className="min-h-screen py-8">
            <div className="container max-w-[1200px] px-4">
                <div className="h-12 w-48 rounded-md bg-muted/10 animate-pulse" />
                <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
                    {[...Array(10)].map((_, index) => (
                        <div key={index} className="h-32 rounded-md bg-muted/10 animate-pulse" />
                    ))}
                </div>
            </div>
        </main>
