...
create policy if not exists "public_read_active_brands" on public.brands
for select using (status = 'active');

create policy if not exists "public_read_active_offers" on public.offers
for select using (status = 'active');

create policy if not exists "public_read_active_payment_methods" on public.payment_methods
for select using (status = 'active');

create policy if not exists "public_read_site_content" on public.site_content
for select using (true);

-- Orders: public INSERT only
create policy if not exists "public_create_orders" on public.orders
for insert with check (true);

create policy if not exists "public_create_order_items" on public.order_items
for insert with check (true);

-- Analytics events: public INSERT
create policy if not exists "public_insert_analytics" on public.analytics_events
for insert with check (true);

-- Admin/full access for authenticated (simple approach for MVP)
-- Using a simple check auth.uid() IS NOT NULL as "authenticated"

create policy if not exists "auth_full_categories" on public.categories
for all using (auth.uid() is not null) with check (auth.uid() is not null);

create policy if not exists "auth_full_brands" on public.brands
for all using (auth.uid() is not null) with check (auth.uid() is not null);

create policy if not exists "auth_full_offers" on public.offers
for all using (auth.uid() is not null) with check (auth.uid() is not null);

create policy if not exists "auth_full_payment_methods" on public.payment_methods
for all using (auth.uid() is not null) with check (auth.uid() is not null);

create policy if not exists "auth_full_site_content" on public.site_content
for all using (auth.uid() is not null) with check (auth.uid() is not null);

create policy if not exists "auth_full_orders" on public.orders
for all using (auth.uid() is not null) with check (auth.uid() is not null);

create policy if not exists "auth_full_order_items" on public.order_items
for all using (auth.uid() is not null) with check (auth.uid() is not null);

create policy if not exists "auth_full_order_status_history" on public.order_status_history
for all using (auth.uid() is not null) with check (auth.uid() is not null);

create policy if not exists "auth_full_analytics" on public.analytics_events
for all using (auth.uid() is not null) with check (auth.uid() is not null);
...
begin
  ...
  insert into public.orders(
    order_number, customer_name, customer_email, customer_whatsapp,
    payment_method_id, payment_method_snapshot, total_amount, currency
  ) values (
    public.generate_order_number(), p_customer_name, p_customer_email, p_customer_whatsapp,
