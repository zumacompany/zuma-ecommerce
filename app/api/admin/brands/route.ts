...
export async function GET() {
  try {
    ...
    const { data, error } = await supabaseAdmin.from('brands').select('id, name, slug, status, logo_path, description_md, category:categories(id, name)').order('name')
...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from('brands').insert(insertObj).select('id, name, slug, status, category:categories(id, name), logo_path').maybeSingle()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
